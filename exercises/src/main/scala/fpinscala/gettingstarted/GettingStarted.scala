...
object MyModule {
  ...
  def fib(n: Int): Int = {
    //todo make sure the fib number lines up with n
    @tailrec
    def go(previous: Int, current: Int, acc: Int): Int ={
      if(acc == n) current
      else go(current, previous + current, acc + 1)
    }
    go(0, 1, 0)
  }
...
object PolymorphicFunctions {
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    println(isSorted[Int](Array(1,2,3,4,4,3,5,6,7), _ <= _))
  }
}
